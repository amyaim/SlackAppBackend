...
@Service
public class WebTechNewsService {
    public TechWebNews getTechNews(){

        List<TechWebNews> articles = new ArrayList<TechWebNews>();
        RestTemplate restTemplate = new RestTemplate();
        String jsonStr = restTemplate.getForObject("https://newsapi.org/v2/top-headlines?sources=the-next-web&apiKey=51a5799e4e8648f7833d95701b3a716c" , String.class);

        try {
            JSONObject json  = new JSONObject(jsonStr);
            JSONArray main  = json.getJSONArray("articles");
            for (int i = 0; i < main.length(); i++) {
                TechWebNews article = new TechWebNews();
        ...
        TechWebNews articles1 = articles.get(index);
        return  articles1;
